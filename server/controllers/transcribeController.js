...
const SERVER_PATH = process.env.SERVER_PATH || path.resolve(__dirname, "..");
        ...
        fileStream.on("close", () => {
            ...
            res.sendFile(filePath, () => {
                fs.unlink(filePath, (err) => {
                    if (err) {
                        console.error(
                            `Error deleting temporary file ${filePath}: ${err}`
                        );
                    }
                });
            });
...
const textToSpeechDataController = async (req, res) => {
    ("TEXT TO SPEECH DATA CONTROLLER");
    const text = req.body;
    const params = {
        OutputFormat: "json",
                    ...
                    if (code === 0) {
                        ...
                        res.sendFile(outputFilePath, () => {
                            fs.unlink(filePath, (err) => {
                                if (err) {
                                    console.error(
                                        `Error deleting temporary files: ${err}`
                                    );
                                }
                            });
                            fs.unlink(trimmedFilePath, (err) => {
